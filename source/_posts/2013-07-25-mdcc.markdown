...
MDCC introduces a new cross data center transaction protocol for
transactions over key-value stores (no range scans) that uses many
forms of paxos to achieve 1 synchronous cross-data center message
in the common case.


* >100ms Latencies between data centers
* 2PC uses too many round trips
  * Megastore uses paxos per-shard.  Heavy conflicts reduces performance to 4 xacts per second
  * Spanner has snapshot consistency, but writes are still slow

###MDCC Goals


**Read commited isolation**: meaning transactions never read records
...
The MDCC paper flows by identifying issues with the best solution proposed so far, and 
introducing a new technique to resolve the issues.  This post will flow in a similar manner.

### Per-record Paxos

Megastore is slow because it runs paxos at a per shard granularity.
What if we ran paxos per record?
...
The following sequence of action could block all nodes participating in a transaction.
...
which amortizes the cost of electing a master to 0 (assuming the masters are stable).
...
* 1 asynchronous roundtrip (can be bundled with other messages) to make commit visible


### Fast-paxos

We still need two synchronous roundtrips -- one to contact the master if it is in 
another datacenter, and one to send the options.  Can we avoid the former message?
...
None of us have read the fast-paxos paper so we were ill-equipped
...
Normal paxos only allows 1 value update per round.  Generalized
paxos allows M as long as those values are commutative.  For example,
if in the same round David wants to increment record A by 1 and
Jane wants to increment record A by 1, then the value of A is 2
regardless of the log order on any of the quorum nodes.
...
The following were derived from first principles.

The fast-paxos protocol was described (briefly) in the paper as

> Any app-server can propose an option directly to the storage (quorum) nodes, which in turn
> promise only to accept the first proposed option.  Simple majority quorums, however, are
> no longer sufficient to leran a value and ensure safeness...
> If a proposer receives an acknowledgment from a fast quorum, the value is safe and 
> guaranteed to be committed.

A classical paxos quorum requires the property that for any two quorums, Qc1 and Qc2, 

  Qc1 INTERSECT Qc2 != Empty set

For fast paxos it has the requirement that for a fast quorum, Qf,

  Qf1 INTERSECT Qf2 INTERSECT Qc != empty set


One of our questions was why a simple majority (Qc) is insufficient and we 
need a Qf > Qc. The following is an example of when it fails.


Recall that Safety in paxos depends on two nontriviality and
consistency.  Consistency states that "At most one value can be
learned".  One example is that even with *f* failures, a committed
log position stays committed.


Lets say Qf = Qc.  Consider the following logical proposals and
their quorums where each column is the state of an acceptor (1-5),
and each row is the state at one paxos round.  C1, C2, C3 are clients
(proposers).  C1 commits A and manages to communicate with acceptors
1-3, C2 commits B and so on.
